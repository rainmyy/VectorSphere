...
const (
	...
	SharedMemName = "Global\\VectorSphereL2Cache"
	// HeaderSize 头部大小
	HeaderSize = 8
)
...
type SharedMemory struct {
	memory     []byte           // 内存映射区域
	mapHandle  syscall.Handle   // 文件映射句柄
...
func NewSharedMemory() (*SharedMemory, error) {
	...
	if err != nil {
		// 如果创建失败，尝试打开现有的共享内存
		// Windows环境下，使用CreateFileMapping打开已有的共享内存
		// 当共享内存已存在时，CreateFileMapping会打开它而不是创建新的
		h, err = syscall.CreateFileMapping(
			syscall.InvalidHandle,                   // 使用页面文件
			nil,                                     // 默认安全属性
			syscall.PAGE_READWRITE,                  // 读写权限
...
func (sm *SharedMemory) Close() error {
	if sm.memory != nil {
		...
		if err != nil {
			return formatError("无法解除共享内存映射", err)
		}
	}

	if sm.mapHandle != 0 {
		err := syscall.CloseHandle(sm.mapHandle)
		if err != nil {
			return formatError("无法关闭共享内存句柄", err)
		}
	}
